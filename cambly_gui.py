...
<body>
  ...
  <div class="actions">
    ...
    <label class="checkbox"><input type="checkbox" id="loginMode">首次登录模式(Playwright 弹浏览器手动登录)</label>
    <span class="status"><span class="badge idle" id="statusBadge">空闲</span> <span id="elapsedInfo" style="font-variant-numeric: tabular-nums;"></span></span>
  </div>
...
<script>
  ...
  startBtn.addEventListener('click', async () => {
    ...
    try {
      const r = await (await fetch('/api/start_export', {
        ...
        body: JSON.stringify({urls, out_dir: outDir, login}),
      })).json();
      if (!r.ok) {
  ...
  (async () => {
    ...
    appendLog('info', '提示:首次使用请先在终端跑一次 python3 cambly_export.py <url> --login,或勾选"首次登录模式"再启动。');
